...
def run_vulnerable_bob():
    print("=========================================================")
    print("         BOB - VULNERABLE DH SERVER (PORT 8080)          ")
    print("=========================================================")
    print(f"[Bob] Private key x: {bob_x}")
    print(f"[Bob] Public key B:  {bob_B}\n")

    # Bind the socket to port 8080 on localhost
    ...
    try:
        ...
        print(f"[Bob] Error binding to port 8080: {e}")
        return

    server.listen(5)
    print("[Bob] Listening for connections on 127.0.0.1:8080...")
    ...
    while True:
        try:
            ...
            # --- NO VALIDATION PERFORMED (VULNERABLE) ---
            # Bob directly computes the shared secret using the unvalidated key y
            K = pow(y, bob_x, p)
        ...
        if choice == 'yes':
            print("[Bob] Stopping vulnerable server.")
            break
